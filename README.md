...
```
orion_muse/
├── observations/              # Raw input .fits data IMPORTANT: THIS FOLDER HAS BEEN OMMITED BECAUSE OF ITS SIZE
...
- `git-lfs` (for large `.json` and `.fits` files)

Install `papermill` and Git LFS:
...
Feel free to open issues or fork the repository to contribute or build upon this work.
